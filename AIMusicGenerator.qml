...
MuseScore {
    ...
    version: "1.0"
    description: "A plugin that communicates with an RNN to generate new notes based on a selected seed melody"
    pluginType: "dialog"
    FileIO {
        id: readTest
        source: filePath
    ...
    /******************************************/
    //property var currentModified: 0
    Window {
        id: main
        visible: true
        width: 400
        ...
        Rectangle {
            ...
            Button {
                ...
                onClicked: {
                    ...
                    var filePath = "/tmp/musescore_generator/in.csv"
                    writeTest.source = filePath;
                    console.log("Writing to: " + filePath);
                    var filePath2 = "/tmp/musescore_generator/out.csv" //set path to read in from the model
                    readTest.source = filePath2;
                    if (!readTest.exists()) {
                        readTest.write("");
                        console.log("writing!");
                    }
                    var initialModified = readTest.modifiedTime(); //get time that file was modified
                    console.log("modified time:" + initialModified);
                    ...
                    var inputTxt = "";
                    readTest.source = filePath2;
                    console.log("modified time:" + readTest.modifiedTime());
                    console.log("Reading from: " + filePath2);
                    
                    /*LOOP TO CHECK IF THERE WAS A MODIFICATION IN MODEL OUTPUT FILE */
                    var wasModified = false;
                    while (wasModified == false) {
                        //setTimeout(getFileTimestamp,  1000);
                        var currentModified = getFileTimestamp();
                        wasModified = (initialModified != currentModified);
                        console.log("wasModifed val: " + wasModified);
                    }
                    ...
                    for (var i = 0; i < strArray.length; i++) {
                        ...
                        if (lineArr[2]) {
                            ...
                            if (isNote && (velocity != 0)) {
                                ...
                                cursorWrite.rewindToTick(adjustedTick); //set cursor position to start of note
                                cursorWrite.staffIdx = track - 1 + startStaff; //set cursor staff to staff of note
            ...
            SpinBox { //SETS HOW MANY MEASURES THE USER WANTS TO GENERATE
                id: measureCounter
                x: 50 * main.scaleFactor
            ...
            Text { //displayed above Complexity slider
                id: temperatureTitle
                text: "Complexity"
            ...
            Button { //INCREASES THE SIZE OF THE WINDOW
            ...
            Button { //DECREASES THE SIZE OF THE WINDOW
    ...
    function moveCursorToTick(cursor, tick) { //moves cusor to beginning tick because it may be inaccessible from the normal cursor position within the selection
    ...
    function getFileTimestamp() {//waits for 1 second and then checks if the model file has changed
        var cmd = "sleep 1" //way to get around there being no real sleep function in musescore API
        proc.start(cmd);
        var val = proc.waitForFinished(1200);
        return readTest.modifiedTime(); //get time that file was modified
    }
}
